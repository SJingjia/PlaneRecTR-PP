...
def eval_plane_recall_depth(predSegmentations, gtSegmentations, predDepths, gtDepths, pred_plane_num, num_queries, threshold=0.5):
    predNumPlanes = pred_plane_num  # actually, it is the maximum number of the predicted planes

    if num_queries in np.unique(gtSegmentations):  # in GT plane Seg., number '20' indicates non-plane
