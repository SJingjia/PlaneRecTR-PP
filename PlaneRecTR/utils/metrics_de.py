...
def evaluateDepths(predDepths, gtDepths, pred_mask, gt_mask, num_queries, file_name = None, printInfo=False):
    """Evaluate depth reconstruction accuracy"""
    predDepths = predDepths.copy()
    gtDepths = gtDepths.copy()
    gt_mask = gt_mask.copy()
    gt_mask = cv2.resize((gt_mask<num_queries).astype(np.uint8), (640, 480)) > 0 
    pred_mask = pred_mask.copy()
    pred_mask = cv2.resize((pred_mask<num_queries).astype(np.uint8), (640, 480)) > 0 
