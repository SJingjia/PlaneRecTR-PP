...
def get_cone_edges(position, lookat, vertical):
    def get_cone_verts(position, lookat, vertical):
        ...
        # print("set came size for pose compare..........")
        # right *= 0.2  # for pose comare (mp3d)
        # lookat *= 0.2
        # right *= 0.2  # for pose comare (scannet)
        # lookat *= 0.2
        # top *= 0.05
        # right *= 0.15  # for teaser
        # lookat *= 0.15
        # print("set came size for 3D plane..........")
        right *= 0.4  # for 3D plane
        lookat *= 0.4
...
def Quaternion2AxisAngle(quats):
    """
    input:
     -- quats: [4,]
    """
...
def Pose2RT(pose_mat):
    ...
    cur_angle = matrix2angle(pose_mat[:, :3, :3])
