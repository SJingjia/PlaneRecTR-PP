...
def visualizationBatch(root_path, idx, info, data_dict, num_queries,
    ...
    if save_segmentation:
        ...
        blend_seg_path1 = os.path.join(root_path, '%s_seg_%s_blend1.png' % (idx, info))
        cv2.imwrite(blend_seg_path, blend_seg)
        cv2.imwrite(blend_seg_path1, seg)
    ...
    if save_ply:
        ...
        for dkey in data_dict.keys():
            if 'depth' in dkey:
                ...
                writePLYFile(root_path, idx, dkey + '_' + info, depth, segmentation, seg, 0, cam, K_inv_dot_xy_1)
