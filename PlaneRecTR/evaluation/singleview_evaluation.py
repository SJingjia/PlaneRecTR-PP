...
class SingleViewEvaluator(DatasetEvaluator):
    """
    ...
    def __init__(
        ...
        *,
        ...
        image_size = (480, 640),
        # predict_poses = True,
        # outputdir_for_res = None,
    ):
        self._logger = logging.getLogger(__name__)
        if num_planes is not None:
            self._logger.warn(
                "SingleViewEvaluator(num_planes) is deprecated! It should be obtained from metadata."
            )
        ...
        # self.outputdir_for_res = outputdir_for_res
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "single" in self._dataset_name:
                gt_plane_masks = input["plane_masks"].tensor.to(self._cpu_device).numpy()
                ...
                for idx,gpm in enumerate(gt_plane_masks):
                    gt[gpm==1]=idx
                gt_plane_depth = torch.sum(input["plane_depths"], dim = 0).to(self._cpu_device).numpy()
                gt_ori_depth = torch.sum(input["depths"], dim = 0).to(self._cpu_device).numpy()
                gt_params = input["params"].to(self._cpu_device).numpy()   
            else:
            ...
            if self.vis:
                ...
                if "single" in self._dataset_name:
                    image = input["image"].to(self._cpu_device).numpy().transpose((1,2,0)) # (h,w,3)
                    file_name = "_".join(input["file_name"].split("/")[6:]).split(".")[0]
    ...
    def evaluate(self):
        ...
        # # self._logger.info({"num_images", num_images})
        # print("num_images:", num_images)

        if self._output_dir:
            PathManager.mkdirs(self._output_dir)

        SingleViewEvaluator.eval_iter += self.eval_period
        res = {}
        ...
        vis_path = pjoin(self._output_dir, "vis_" + str(SingleViewEvaluator.eval_iter))
