...
def get_plane_parameters(plane, plane_nums, segmentation, num_queries):

    valid_region = segmentation < num_queries # add 21 for pading
    ...
    # plane_instance_parameter = np.concatenate((plane, np.zeros((num_queries - plane.shape[0], 3))), axis=0)
    plane_instance_parameter = None
...
def plane2depth(K_inv_dot_xy_1,  tfm_K_inv_dot_xy_1, S_K_inv_dot_xy_1, dataset_K_inv_dot_xy_1,
                plane_parameters, num_queries, segmentation, gt_depth, h=192, w=256):
    ...
    depth_map[segmentation >= num_queries] = gt_depth[segmentation >= num_queries]
    ...
    # replace non planer region depth using sensor depth map
    tfm_depth_map[segmentation >= num_queries] = gt_depth[segmentation >= num_queries]

    S_depth_map = 1. / np.sum(S_K_inv_dot_xy_1.reshape(3, -1) * plane_parameters.reshape(3, -1), axis=0)
    S_depth_map = S_depth_map.reshape(h, w)
    # replace non planer region depth using sensor depth map
    S_depth_map[segmentation >= num_queries] = gt_depth[segmentation >= num_queries]
    ...
    # replace non planer region depth using sensor depth map
    dataset_depth_map[segmentation >= num_queries] = gt_depth[segmentation >= num_queries]
...
def test_tfm_K(dataset_K_inv_dot_xy_1,
               ...
               tfm_segmentation, gt_segmentation, num_planes, num_queries,
    ...
    plane_depth, tfm_plane_depth, S_plane_depth, dataset_plane_depth = plane2depth(K_inv_dot_xy_1, tfm_K_inv_dot_xy_1, S_K_inv_dot_xy_1, dataset_K_inv_dot_xy_1,
                                                              plane_parameters, num_queries, tfm_segmentation, tfm_depth)
    ...
    visualizationBatch(root_path = plane_vis_path, idx = idx, info = "gt",
                        data_dict = gt_vis_dict, num_queries = num_queries, save_image = True, save_segmentation = True,
    ...
    visualizationBatch(root_path = plane_vis_path, idx = idx, info = "tfm",
                        data_dict = tfm_vis_dict, num_queries = num_queries, save_image = True, save_segmentation = True,
                        save_depth = True, save_ply = True, save_cloud = False)
...
def after_transform_apply_K_inv_dot_xy_1(tfm_gt_K_inv_dot_xy_1, tfm_gt_segmentation, tfm_gt_depth, plane, 
    ...
    plane_parameters, valid_region, plane_instance_parameter = \
        get_plane_parameters(plane, num_planes, tfm_gt_segmentation, num_queries)
